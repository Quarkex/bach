set_action "compose" "project_name instance_name"                              \
"Send a docker-compose action to an already generated instance of a project"   \
"Usage:"                                                                       \
...
compose(){(
    ...
    if [ ! -d "$instance_folder" ]; then
        ...
        cd "$instance_folder";
        docker-compose $@ $composed_options;
        return $?
    fi
)}
